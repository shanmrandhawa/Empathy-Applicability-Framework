...
def load_data(filename):
    ...
    Assumes there's a 'pat_query' column for text and 'IA' or 'interpretation' for label.
    Renames 'IA' -> 'interpretation' if needed.
    """
    df = pd.read_csv(filename)
    df['IA'] = df['IA'].map({'Not Applicable': 0, 'Applicable': 1})

    # If your CSV has a column 'IA' instead of 'interpretation', rename:
    if 'IA' in df.columns and 'interpretation' not in df.columns:
        df = df.rename(columns={'IA': 'interpretation'})
...
def tokenize_function(examples):
    ...
    For binary classification, expects 'interpretation' column as label (0 or 1).
    """
    # Tokenize text
    tokenized = tokenizer(
        examples['pat_query'],
    ...
    # Rename 'interpretation' -> 'labels' for Trainer compatibility
    if 'interpretation' in examples:
        tokenized['labels'] = [int(label) for label in examples['interpretation']]
    return tokenized
...
# For Human version, load the IA_train.csv, 
# for Autonomous version, load the unseen_EA_IA_train_autonomous.csv
train_dataset = load_data('IA_train.csv')
val_dataset   = load_data('IA_eval.csv')
test_dataset  = load_data('IA_test.csv')
...
training_args = TrainingArguments(
    ...
    run_name="empathyIA"        # W&B run name
)
...
test_df = pd.read_csv("IA_test.csv")
test_df["Predicted_Label"] = pred_labels

# Save to CSV
output_filename = "test_output_ia.csv"
