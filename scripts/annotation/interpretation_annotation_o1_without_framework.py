...
ANNOTATION_SCHEMA = {
    "instruction": "Read the patient query and decide whether interpretations are necessary in the response. "
                   "Interpretations refer to the communication of an understanding of the patient’s feelings "
                   "(explicit or implied) and/or experiences (contextual factors) inferred from their query. "
                   "If interpretations are necessary, mark it as 'Interpretations Applicable'. "
                   "If not, mark it as 'Interpretations Not Applicable'. Think carefully and be consistent.",
    "output_format": "json"
}
    ...
    try:
        for index, row in df.iloc[start:end].iterrows():
            ...
            if applicability != "Error":
              df.at[index, 'IA_applicability'] = applicability
              time.sleep(1)  # To avoid rate limits
            else:
              df.at[index, 'IA_applicability'] = "Error"
              time.sleep(1)  # To avoid rate limits
    except Exception as e:
...
df.to_csv("interpertations_annotations_o1_zeroshot.csv", index=False)
